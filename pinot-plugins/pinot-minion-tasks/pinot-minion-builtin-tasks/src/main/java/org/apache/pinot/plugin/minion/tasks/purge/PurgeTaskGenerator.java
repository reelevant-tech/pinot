    ...
    for (SegmentZKMetadata segmentZKMetadata : notpurgedSegmentsZKMetadata) {
      ...
      if (segmentZKMetadata.getCustomMap() != null) {
        tsLastPurge = Long.valueOf(segmentZKMetadata.getCustomMap()
            .getOrDefault(MinionConstants.PurgeTask.TASK_TYPE + MinionConstants.TASK_TIME_SUFFIX, "0"));
      } else {
        tsLastPurge = 0L;
      }
      ...
      configs.put(MinionConstants.PurgeTask.MATCHING_COLUMN_NAME, taskConfigs.get(MinionConstants.PurgeTask.MATCHING_COLUMN_NAME));
      configs.put(MinionConstants.PurgeTask.MATCHING_COLUMN_VALUES, taskConfigs.get(MinionConstants.PurgeTask.MATCHING_COLUMN_VALUES));
